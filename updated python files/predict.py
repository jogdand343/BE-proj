...
def predict(x,d):
    ...
    plt.grid()
    plt.title('Weekly Interest at ' + x )
    ...
    #p = " ".join(p)
    print("Predicted result based on yearly trend is " + p +"%")
    print("Actual level of interest for the week is " + s + "") 

    
    """from sklearn.externals import joblib
    joblib.dump(lin_reg_2, 'model.pkl')
    print("Model dumped!")

    # Load the model that you just saved
    lin_reg_2 = joblib.load('model.pkl')
    model_columns = list(X_poly.columns)
